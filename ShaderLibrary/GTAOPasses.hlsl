...
half4 FragGTAO(Varyings IN): SV_Target
{
    ...
    return GTAO(uv);
   // bentNormal = bn_ao.rgb;//SpheremapEncodeNormal(bn_ao.rgb);
  //  ao_bent = half2(bn_ao.a, depth);
}
...
half4 FragGTAOSpatialX(Varyings IN) : SV_Target
{
    ...
    //half4 AO = BilateralClearUp(TEXTURE2D_ARGS(_GTAOTexture, sampler_linear_clamp), _RenderSizeParams.xy, uv);
   half4 AO = BilateralBlur(BentNormal_ao, linearDepth,uv, half2(1 / _RenderSizeParams.x, 0));//w?
    return AO;//half4(BentNormal_ao.xyz, AO);
}
...
half2 FragGTAOSpatialY(Varyings IN) : SV_Target
{
    ...
    half linearDepth = Linear01Depth(depth, _ZBufferParams);
    half4 BentNormal_ao = SAMPLE_TEXTURE2D(_GTAOTexture, sampler_linear_clamp, uv);
    half4 AO = BilateralBlur(BentNormal_ao.a, linearDepth,uv, half2(0, 1 / _RenderSizeParams.y));//w?
